...
contract MultiSigWallet{
    ...
    // to do assignment
    //Revert Approval
    function revertApproval(uint256 tID)external onlyOwner isTransaction(tID) {
        //check
        require( transactions[tID].executed == true, "not executed");
        transactions[tID].executed =false;
        // go out of the contract
        (bool status, ) = transactions[tID].to.call{value: transactions[tID].value}( transactions[tID].data);
